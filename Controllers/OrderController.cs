...
namespace MVCAdminApplication.Controllers
{
    public class OrderController : Controller
    {
        ...
        public FileContentResult CreateInvoice(string id)
        {
            ...
            document.Content.Replace("{{OwnerName}}", result.Owner.FirstName + " "+ result.Owner.LastName);

            StringBuilder sb = new StringBuilder();
            var total = 0.0;
            foreach (var item in result.ProductInOrders)
            {
                sb.AppendLine("Product " + item.OrderedProduct.Movie.MovieName + " has " + item.Quantity + " tickets with $" + item.OrderedProduct.Price+" per ticket and $" + (item.Quantity * item.OrderedProduct.Price) + " total");
